...
import { getBoatBookings, getBookingsBaseType, getExperienceBookingsBaseType, updateBookingStatus } from '../../services/api/bookedService';
...
const ExperienceBookingGlobal
  = ({ yachtsType }) => {
    ...
    const fetchBookings = async () => {
      try {
        setLoading(true);
        const data = await getExperienceBookingsBaseType(1, yachtsType);
        setBookings(data);
      } catch (err) {
    ...
    const handleRowClick = (booking, experience) => {
      setSelectedBooking({ booking, experience });
      setIsModalOpen(true);
    };
      ...
      if (action === "approved") {
        ...
        try {
          let payload = {
            ...
            user_id: userId,
            booking_type: yachtsType == "f1-exp" ? "exp" : "exp",
            action: "approved",
          ...
          <CardHeader floated={false} shadow={false} className="rounded-none">
            <div className="mb-4">
              <Typography variant="h3" color="black" className="font-sora">
                ...
                {yachtsType == "regular-exp" ? " Experience Bookings" : yachtsType == "f1-exp" ? " F1 Experience Bookings" : ""}

              </Typography>
              <Typography variant="small" color="gray" className="mt-1">
                {yachtsType == "regular-exp" ? "  Manage your Experience bookings" : yachtsType == "f1-exp" ? "  Manage your F1 Experience bookings" : ""}
                ...
                <table className="w-full text-left border-collapse rounded-lg overflow-hidden shadow-sm bg-white">
                  <thead className="bg-black text-white text-sm uppercase font-medium">
                    <tr className=' '>
                      ...
                      <th className="border-b border-blue-gray-100 p-4">Experience</th>
                      <th className="border-b border-blue-gray-100 p-4">Booking Date</th>
                      {/* {yachtsType == "regular-exp" ? <th className="border-b border-blue-gray-100 p-4">Duration</th> : yachtsType == "f1-exp" ? "" : ""} */}
                    ...
                    {currentBookings.map((item) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => handleRowClick(booking, experience)}
                        >
                          <td className="p-4">{booking?.User || booking.user_id || 'Guest'}</td>
                          <td className="p-4">
                            <div>
                              <div className="font-medium">{experience?.name ? experience?.name : experience?.title ? experience?.title : ""}</div>
                          ...
                          <td className="p-4 flex items-center gap-2">
                            ...
                            >
                              ...
                              {yachtsType == "regular-exp" ? isApproved ? "Approved" : paymentApproveLoading ? `Accept` : `Accept` : yachtsType == "f1-exp" ? isApproved ? "Approved" : paymentApproveLoading ? `Accept` : `Accept` : ""}
                            </button>
                            ...
                            >
                              ...
                              {yachtsType == "regular-exp" ? isCancelled ? "Cancelled" : paymenCancelLoading ? `Cancel` : `Cancel` : yachtsType == "f1-exp" ? isCancelled ? "Cancelled" : paymenCancelLoading ? `Cancel` : `Cancel` : ""}
                            </button>
                          </td>
                        </tr>
                      );
                    ...
                    <>
                      ...
                      <DialogHeader className="flex items-center justify-between border-b border-gray-200 p-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            {yachtsType == "regular-exp" ? <Typography variant="h5">Booking Details</Typography> : yachtsType == "f1-exp" ? <Typography variant="h5">F1 Booking Details</Typography> : ""}
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              <InfoRow
                                ...
                                value={selectedBooking?.experience?.name ? selectedBooking?.experience?.name : selectedBooking?.experience?.title ? selectedBooking?.experience?.title : ""}
                              />
                              <InfoRow
                                icon={MapPin}
                                label="Location"
                                value={selectedBooking.experience?.location}
                              />
                              <InfoRow
                                icon={Users}
                                label="Capacity"
                                value={`${selectedBooking.experience?.capacity} guests`}
                              />
                              {/* <InfoRow
                              icon={ClipboardIcon}
                              label="Notes"
                              value={selectedBooking.experience?.notes || "No notes available"}
                            /> */}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              ...
                              /> : yachtsType == "f1-exp" ? <InfoRow
                                icon={CalendarDaysIcon}
                                label="Booking Date"
                                value={formatDateWithTimes(selectedBooking?.booking?.start_date)}
                              /> : ""}
...
export default ExperienceBookingGlobal
  ;