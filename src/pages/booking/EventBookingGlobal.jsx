...
const EventBookingGlobal
  = ({ yachtsType }) => {
    ...
    const fetchBookings = async () => {
      try {
        setLoading(true);
        const data = await getEventBookingsAll(1);
        setBookings(data);
      } catch (err) {
    ...
    const handleRowClick = (booking, event) => {
      setSelectedBooking({ booking, event });
      setIsModalOpen(true);
    };
          ...
          <CardHeader floated={false} shadow={false} className="rounded-none">
            <div className="mb-4">
              <Typography variant="h3" color="black" className="font-sora">
                ...
                {yachtsType == "all-events" ? " Events Bookings" : yachtsType == "f1-exp" ? " F1 Events Bookings" : ""}

              </Typography>
              <Typography variant="small" color="gray" className="mt-1">
                {yachtsType == "all-events" ? "  Manage your Events bookings" : yachtsType == "f1-exp" ? "  Manage your F1 Events bookings" : ""}
                ...
                <table className="w-full text-left border-collapse rounded-lg overflow-hidden shadow-sm bg-white">
                  <thead className="bg-black text-white text-sm uppercase font-medium">
                    <tr className=' '>
                      ...
                      {/* {yachtsType == "all-events" ? <th className="border-b border-blue-gray-100 p-4">Duration</th> : yachtsType == "f1-exp" ? "" : ""} */}
                    ...
                    {currentBookings.map((item) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => handleRowClick(booking, event)}
                        >
                          <td className="p-4">{booking?.User || booking.user_id || 'Guest'}</td>
                          <td className="p-4">
                            <div>
                              <div className="font-medium">{event?.name ? event?.name : event?.title ? event?.title : ""}</div>
                          ...
                          <td className="p-4 flex items-center gap-2">
                            ...
                            >
                              ...
                              {yachtsType == "all-events" ? isApproved ? "Approved" : paymentApproveLoading ? `Accept` : `Accept` : yachtsType == "f1-exp" ? isApproved ? "Approved" : paymentApproveLoading ? `Accept` : `Accept` : ""}
                            </button>
                            ...
                            >
                              ...
                              {yachtsType == "all-events" ? isCancelled ? "Cancelled" : paymenCancelLoading ? `Cancel` : `Cancel` : yachtsType == "f1-exp" ? isCancelled ? "Cancelled" : paymenCancelLoading ? `Cancel` : `Cancel` : ""}
                            </button>
                          </td>
                        </tr>
                      );
                    ...
                    <>
                      ...
                      <DialogHeader className="flex items-center justify-between border-b border-gray-200 p-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            {yachtsType == "all-events" ? <Typography variant="h5">Booking Details</Typography> : yachtsType == "f1-exp" ? <Typography variant="h5">F1 Booking Details</Typography> : ""}
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              <InfoRow
                                icon={Anchor}
                                label="event Name"
                                value={selectedBooking?.event?.name ? selectedBooking?.event?.name : selectedBooking?.event?.title ? selectedBooking?.event?.title : ""}
                              />
                              <InfoRow
                                icon={MapPin}
                                label="Location"
                                value={selectedBooking.event?.location}
                              />
                              {/* <InfoRow
                                icon={Users}
                                label="Capacity"
                                value={`${selectedBooking.event?.capacity} guests`}
                              /> */}
                              {/* <InfoRow
                              icon={ClipboardIcon}
                              label="Notes"
                              value={selectedBooking.event?.notes || "No notes available"}
                            /> */}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              ...
                              /> : yachtsType == "f1-exp" ? <InfoRow
                                icon={CalendarDaysIcon}
                                label="Booking Date"
                                value={formatDate(selectedBooking?.event?.start_date)}
                              /> : ""}
...
export default EventBookingGlobal
  ;