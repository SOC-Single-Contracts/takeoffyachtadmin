...
const Boats = () => {
  ...
  useEffect(() => {
    const fetchData = async (id) => {
      try {
        const response = await fetch(`https://api.takeoffyachts.com/yacht/get_yacht/${id}`);
      ...
      <Card className="h-full w-full p-4">
        ...
        <CardBody className="overflow-auto px-0">
          <table className="w-full text-left border-collapse rounded-lg overflow-hidden shadow-sm bg-white mb-6">
            ...
            <tbody>
              {yachtData.map((item) => (
                <tr key={item.yacht.id} onClick={() => openModal(item.yacht)}>
                  <td className="p-4">{item.yacht.name}</td>
                  <td className="p-4">{item.yacht.location}</td>
                  <td className="p-4">${item.yacht.per_hour_price}</td>
                  <td className="p-4">
